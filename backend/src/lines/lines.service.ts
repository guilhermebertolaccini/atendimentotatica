...
export class LinesService {
  ...
  async assignOperatorToLine(lineId: number, userId: number): Promise<void> {
    ...
    return await this.prisma.$transaction(async (tx) => {
      ...
      // Buscar informações do usuário para verificar se é agente
      ...
      // Agentes (role: 'agente') não têm limite de operadores por linha
      if (!sharedLineMode && user.role !== 'agente') {
        // Linhas reserva aceitam apenas 1 operador, linhas normais aceitam 2
        // Esta limitação NÃO se aplica a agentes (podem ser múltiplos)
