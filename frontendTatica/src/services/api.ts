...
export const agentsService = {
  ...
  list: async (includeInactive: boolean = false): Promise<Agent[]> => {
    return apiRequest(`/agents?includeInactive=${includeInactive}`);
  },

  // Agentes de uma linha específica
  getByLine: async (lineId: number): Promise<Agent[]> => {
    return apiRequest(`/agents/line/${lineId}`);
  },

  // Agentes online de uma linha
  getOnlineByLine: async (lineId: number): Promise<Agent[]> => {
    return apiRequest(`/agents/line/${lineId}/online`);
  },

  // Linha do agente logado
  getMyLine: async (): Promise<AgentLine | null> => {
    return apiRequest('/agents/me/line');
  },

  // Conversas do agente logado
  getMyConversations: async (): Promise<any[]> => {
    return apiRequest('/agents/me/conversations');
  },

  // Estatísticas do agente logado
  getMyStats: async (startDate?: string, endDate?: string): Promise<AgentStats> => {
    const params = new URLSearchParams();
    if (startDate) params.append('startDate', startDate);
    if (endDate) params.append('endDate', endDate);
    const query = params.toString() ? `?${params.toString()}` : '';
    return apiRequest(`/agents/me/stats${query}`);
  },

  // Detalhes do agente logado
  getMyDetails: async (): Promise<any> => {
    return apiRequest('/agents/me/details');
  },

  // Detalhes de um agente específico
  getById: async (agentId: number): Promise<any> => {
    return apiRequest(`/agents/${agentId}`);
  },

  // Estatísticas de um agente específico
  getStats: async (agentId: number, startDate?: string, endDate?: string): Promise<AgentStats> => {
    const params = new URLSearchParams();
    if (startDate) params.append('startDate', startDate);
    if (endDate) params.append('endDate', endDate);
    const query = params.toString() ? `?${params.toString()}` : '';
    return apiRequest(`/agents/${agentId}/stats${query}`);
  },

  // Vincular agente a linha (apenas admin)
  assignToLine: async (agentId: number, lineId: number): Promise<any> => {
    return apiRequest(`/agents/${agentId}/line/${lineId}`, {
      method: 'POST',
    });
  },

  // Remover agente de linha (apenas admin)
  removeFromLine: async (agentId: number, lineId: number): Promise<any> => {
    return apiRequest(`/agents/${agentId}/line/${lineId}`, {
      method: 'DELETE',
    });
  },
  ...
  cleanupExpiredBindings: async (): Promise<{
    success: boolean;
    message: string;
    count: number;
  }> => {
    return apiRequest('/agents/conversations/cleanup', {
      method: 'POST',
    });
  },
};
