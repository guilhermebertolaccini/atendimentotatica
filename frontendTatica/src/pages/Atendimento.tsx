...
export default function Atendimento() {
  ...
  const [typingTimeout, setTypingTimeout] = useState<NodeJS.Timeout | null>(null); // Timeout para stop-typing
  const [isTyping, setIsTyping] = useState(false); // Se está digitando
    ...
    (data: any) => {
      ...
      if (data.mode === "shared") {
        ...
        if (data.conversations && Array.isArray(data.conversations)) {
          ...
          data.conversations.forEach((conv: APIConversation) => {
            const groupKey =
  ...
  useRealtimeSubscription(
    ...
    (data: any) => {
      ...
      toast({
        title: "Novo agente online",
        description: `${data.agentName || "Agente"} entrou na linha`,
        duration: 2000,
      });
    },
    []
  ...
  useRealtimeSubscription(
    ...
    (data: any) => {
      ...
      toast({
        title: "Agente desconectou",
        description: `${data.agentName || "Agente"} saiu da linha`,
        duration: 2000,
      });
    },
    []
  ...
  // Handler para mudanças no input de mensagem (com emissão de eventos de typing)
    ...
    (value: string) => {
      ...
      if (user?.role === "agente" && selectedConversation) {
        ...
        if (value.length > 0) {
          ...
          // Se apagou tudo, parar typing imediatamente
          realtimeSocket.send("stop-typing", {
            contactPhone: selectedConversation.contactPhone,
          });
          setIsTyping(false);
  ...
  return (
    <MainLayout>
      ...
      <div className="h-[calc(100vh-6rem)] flex gap-4">
          ...
          {selectedConversation ? (
            <>
              ...
              {selectedConversation && typingAgents.has(selectedConversation.contactPhone) && (
                <div className="flex items-center gap-2 px-4 py-2 text-sm text-muted-foreground italic border-t border-border/50 bg-muted/20">
        ...
        {/* Agentes Online Sidebar */}
        {isAgentMode && agentesOnline.length > 0 && (
          <Card className="w-64 flex flex-col glass-card">
